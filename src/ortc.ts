...
import { SctpCapabilities, NumSctpStreams, SctpStreamParameters } from './SctpParameters';
...
export function getSendingRemoteRtpParameters(
	...
	if (
		rtpParameters.headerExtensions!.some((ext) => ext.uri === 'http://www.ietf.org/id/draft-holmer-rmcat-transport-wide-cc-extensions-01')
	) {
		for (const codec of rtpParameters.codecs) {
			codec.rtcpFeedback = (codec.rtcpFeedback || []).filter((fb) => fb.type !== 'goog-remb');
		}
	} else if (
		rtpParameters.headerExtensions!.some((ext) => ext.uri === 'http://www.webrtc.org/experiments/rtp-hdrext/abs-send-time')
	) {
		for (const codec of rtpParameters.codecs) {
